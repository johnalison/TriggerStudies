# Compare Inputs between HLT Nominal and HLT Best
# --input1 ROOTFileWithHists_Nominal
# --input2 ROOTFileWithHists_Best
# --output outDirectory
import ROOT
...
p = OptionParser()
p.add_option('--input1',  type = 'string', default = "outBTag.FTKBtagging.ttbar.mwt2.All.root", dest = 'inFile1', help = 'intput File1' )
p.add_option('--input2',  type = 'string', default = "outBTag.FTKBtagging.ttbar.mwt2.All.root", dest = 'inFile2', help = 'intput File2' ) #another input forHLT2
p.add_option('--output', type = 'string', default = "HLTNominalvsBest", dest = 'outDir', help = 'output dir' )
...
#    #NomiLF = getHist(inFile,"caloJets_matchedL",          var,binning,ROOT.kBlack)
#    NomiLF = getHist(inFile,"offJets_matched_L",          var,binning,ROOT.kBlack)
#    BestLF = getHist(inFile,"offJets_matchedJet_L",var,binning,ROOT.kBlack)
#    #BestLF = getHist(inFile,"caloJets_matchedL",var,binning,ROOT.kBlack)
#    #NomiBQ = getHist(inFile,"caloJets_matchedB",          var,binning,ROOT.kRed)
#    NomiBQ = getHist(inFile,"offJets_matched_B",          var,binning,ROOT.kRed)
#    BestBQ = getHist(inFile,"offJets_matchedJet_B",var,binning,ROOT.kRed)
#    #BestBQ = getHist(inFile,"caloJets_matchedB",var,binning,ROOT.kRed)
#
#    xpos = 0.18
#    ypos = 0.6
#    xwidth = 0.3
...
#    leg.AddEntry(NomiBQ,"Offline tracks b-quark jets","L")
#    leg.AddEntry(BestBQ,"HLT PF b-quark jets"    ,"PEL")
#    leg.AddEntry(NomiLF,"Offline tracks light-flavor jets","L")
#    leg.AddEntry(BestLF,"HLT PF light-flavor jets"    ,"PEL")
#
#
#    can = ROOT.TCanvas(var,var)
#    can.cd().SetLogy(setLogy)
#    maxY = max(NomiLF.GetMaximum(),NomiBQ.GetMaximum(),
#               BestLF.GetMaximum(),BestBQ.GetMaximum())
#    if setLogy:
#        NomiLF.SetMaximum(4e0*maxY)
#        NomiLF.SetMinimum(1e-6)
#    else:
#        NomiLF.SetMaximum(1.2*maxY)
#        
#    NomiLF.GetYaxis().SetTitle("Simulated Tracks")
#    NomiLF.GetXaxis().SetTitle(xTitle )
#    NomiLF.Draw("hist")
#    #BestLF.SetMarkerSize(0.75)
#    #BestLF.SetMarkerStyle(21)
#    BestLF.Draw("same pe")
#    NomiBQ.Draw("hist same")
#    #BestBQ.SetMarkerSize(0.75)
#    #BestBQ.SetMarkerStyle(21)
#    BestBQ.Draw("same pe")
#    #leg.Draw("same")
#
...
def doVarRatio(var,binning,xTitle,setLogy=1,minX=None,maxX=None,minY=None):
    if o.doCaloJets:
        NomiLF = getHist(inFile1,"offJets_matchedCaloJet_L",          var,binning,ROOT.kBlack)
        BestLF = getHist(inFile2,"offJets_matchedCaloJet_L",var,binning,ROOT.kBlack)
        NomiBQ = getHist(inFile1,"offJets_matchedCaloJet_B",          var,binning,ROOT.kRed)
        BestBQ = getHist(inFile2,"offJets_matchedCaloJet_B",var,binning,ROOT.kRed)
    # **matchedJet_L & B
    # from Input1 and Input2
    else:
        NomiLF = getHist(inFile1,"offJets_matchedJet_L",          var,binning,ROOT.kBlack)
        BestLF = getHist(inFile2,"offJets_matchedJet_L",var,binning,ROOT.kBlack)
        NomiBQ = getHist(inFile1,"offJets_matchedJet_B",          var,binning,ROOT.kRed)
        BestBQ = getHist(inFile2,"offJets_matchedJet_B",var,binning,ROOT.kRed)
    ...
    if setLogy:
        NomiLF.SetMaximum(4e0*maxY)
        NomiLF.SetMinimum(1.01e-5)
    else:
        NomiLF.SetMaximum(1.2*maxY)

    if not minY == None :
        NomiLF.SetMinimum(minY)
    ...
    if maxX:
        NomiLF.GetXaxis().SetRangeUser(minX,maxX)
        NomiBQ.GetXaxis().SetRangeUser(minX,maxX)
        BestLF.GetXaxis().SetRangeUser(minX,maxX)
        BestBQ.GetXaxis().SetRangeUser(minX,maxX)


    LFRatio = makeRatio(num = BestLF.Clone(),  den = NomiLF.Clone())
    BQRatio = makeRatio(num = BestBQ.Clone(),  den = NomiBQ.Clone())
    ...
    leg = ROOT.TLegend(xpos, ypos, xpos+xwidth, ypos+ywidth)
    leg.SetNColumns(2)
    leg.AddEntry(NomiBQ,"Nominal tracks b-quark jets","L")
    leg.AddEntry(NomiLF,"Nominal tracks light-flavor jets","L")
    leg.AddEntry(BestBQ,"Best cut's tracks b-quark jets"    ,"PEL")
    leg.AddEntry(BestLF,"Best cut's tracks light-flavor jets" ,"PEL")
    ...
    if var in maxDict.keys():
        NomiLF.GetXaxis().SetRangeUser(NomiLF.GetXaxis().GetXmin(),maxDict[var] )
    ...
    ratio_axis = NomiLF.Clone()
    #ratio_axis.GetYaxis().SetTitle("PF to Calo")
    ratio_axis.GetYaxis().SetTitle("Best to Nominal")
    ratio_axis.GetXaxis().SetTitle(NomiLF.GetXaxis().GetTitle())
    ...
    LFRatio.GetYaxis().SetTitle("Best to Nominal")
    ...
    line = ROOT.TLine()
    line.DrawLine(NomiLF.GetXaxis().GetXmin(), 1.0, NomiLF.GetXaxis().GetXmax(), 1.0)
