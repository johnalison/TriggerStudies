...
### B-Jet efficiency vs cut given certain LightFlavor efficiency
...
def computesigEff(inFile, var, bkg, sig, dir,workingpoint = 0.7, debug=False):
...
def makeWorkingPointsHists(workingpoint, cuts, sigEffs):
    c1 = ROOT.TCanvas("c1", "B-Jet Efficiency as Light-Flavor Efficiency fixed to "+str(workingpoint))
    c1.SetGrid()
    gr = ROOT.TGraph(len(cuts))
    #print("in make working points hists:", cuts, bkgEffs)
    for i in range(len(cuts)):
        gr.SetPoint(i,cuts[i],sigEffs[i])    
    gr.GetXaxis().SetTitle("TrackPtCut/GeV")
    gr.GetYaxis().SetTitle("B-Jet Efficiency")
    gr.SetLineColor( 2 )
    gr.SetLineWidth( 4 )
    gr.SetMarkerColor( 4 )
    gr.SetMarkerStyle( 21 )
    gr.SetTitle("B-Jet Efficiency as Light Flavor Efficiency fixed to "+str(workingpoint))
    gr.Draw("ALP")
    cmsLine1, cmsLine2 = getCMSText(xStart=0.2,yStart=0.875,subtext="Light Flavor Efficiency fixed to "+str(workingpoint))
    cmsLine1.Draw("same")
    cmsLine2.Draw("same")
    c1.Update()
    ...
    c1.Modified()
    c1.Update()
    c1.SaveAs(o.outDir+"/"+str(workingpoint*100)+".pdf")
...
def main():
    workingpoints = [0.2, 0.1, 0.05, 0.01]
    for workingpoint in workingpoints:
	    ...
	    #print(inFile.GetName())
            sigEff, bkgEff = computesigEff(inFile, "DeepCSV_l", bkg="matchedJet_L", sig="matchedJet_B", dir="offJets", workingpoint = workingpoint)
            cut = nametoCut(f)
            cuts.append(cut)
            sigEffs.append(sigEff)
            bkgEffs.append(bkgEff)
        #print("at"+str(workingpoint)+"  bkgEffs[]:", bkgEffs)
        makeWorkingPointsHists(workingpoint, cuts, sigEffs)
